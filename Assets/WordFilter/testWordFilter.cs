...
public class testWordFilter : MonoBehaviour {
	...
	void downloadWords(bool withReplacements){
		UnityWebRequest w = new UnityWebRequest("http://simple-chat-pro.appspot.com/words.php"+((withReplacements)?"?rep=true":""));
		StartCoroutine(waitForWords(w));
	}
	//function to wait for the word list to download
	IEnumerator waitForWords(UnityWebRequest www){
		yield return www;
		if(www.error == null)
			finishedLoadingWords(www.error);
		else
			Debug.Log(www.error);
	}
